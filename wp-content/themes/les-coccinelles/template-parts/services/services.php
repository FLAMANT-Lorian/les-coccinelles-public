...
<section>
    <div class="px-default py-default grid-default gap-y-10">
        ...
        <?php if ($items): ?>
            <div class="grid grid-cols-4 md:grid-cols-8 lg:grid-cols-10 gap-x-6 gap-y-14 col-span-full xg:col-start-2 xg:col-span-10">
                <?php foreach ($items as $index => $item): ?>
                    <?php if ($item['image']): ?>
                        <div class="service-item flex flex-col items-center relative col-span-full md:col-span-4 md:last:col-span-8 rg:last:col-start-3 rg:last:col-span-4 rl:last:col-start-3 rl:last:col-span-4 lg:odd:col-span-3 lg:even:col-span-4 lg:even:mt-8 ?>">
                        <div class="overflow-hidden">
                            <?= wp_get_attachment_image($item['image']['ID'], 'large', attr: [
                                    'class' => 'h-66 sm:h-80 object-cover object-center trans-all',
                                    'alt' => $item['image']['alt']
                            ]) ?>
                        </div>
                    <?php endif; ?>
                    <?php if ($item['name']): ?>
                        <span class="relative z-1 btn-services -mt-5.75">
                                <?= $item['name'] ?>
                            </span>
                    <?php endif; ?>
                    <?php if ($item['button']): ?>
